    ...
    public static void init(){
        ClientTickEvents.END_CLIENT_TICK.register( client -> {
            while (switchType.wasPressed())
                selectType(MinecraftClient.getInstance());
        });
    }

    private static void selectType(@NotNull MinecraftClient client){
        if (client.player != null)
        {
            ...
            switch (typeNumber){
                case 0:
                    currentType = "none";
                    client.player.playSound(SoundEvents.ITEM_ARMOR_EQUIP_IRON.value(), 1.0F, 1.2F);
                    break;
                case 1:
                    currentType = "player";
                    client.player.playSound(SoundEvents.ITEM_ARMOR_EQUIP_IRON.value(), 1.0F, 1.2F);
...
//            client.player.sendMessage(Text.literal("Targeted to " + currentType + " entities"), true);
        }
    }
